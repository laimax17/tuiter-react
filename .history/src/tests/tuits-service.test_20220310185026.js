...
describe('can retrieve all tuits with REST API', () => {
  ...
  test("can retrieve all tuits with REST API", async() => {
    const creUser1 = await createUser(max);
    const creUser2 = await createUser(sam);
    const creUser3 = await createUser(chan);
    ...
    console.log(newUser2[0]._id);
    console.log(newUser3[0]._id);
    const retrieveTuit1 = allTuits.filter((x) => x.postedBy._id === newUser1[0]._id);
    const retrieveTuit2 = allTuits.filter((x) => x.postedBy._id  === newUser2[0]._id);
    const retrieveTuit3 = allTuits.filter((x) => x.postedBy._id  === newUser3[0]._id);
    
    console.log(retrieveTuit1);
    console.log(retrieveTuit2);
    console.log(retrieveTuit3);
