...
describe('can retrieve all tuits with REST API', () => {
  ...
  test("can retrieve all tuits with REST API", async() => {
    ...
    users.forEach((x) => {
      const currUser = allUsers.filter(user => user.username === x.username);
    ...
    users.forEach((x) => {
      const currTuits = allTuits.filter(tuit => tuit.postedBy === x._id);
