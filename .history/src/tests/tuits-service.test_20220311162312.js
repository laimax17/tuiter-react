...
describe('can retrieve all tuits with REST API', () => {
  ...
  test('can retrieve all tuits from REST API', async () => {
    ...
    expect(retrievedTuits.length).toBeGreaterThanOrEqual(sampleTuits.length);
      console.log(sampleTuits);
      console.log(Promise.resolve(sampleTuits));
    // const tuitsInserted = retrievedTuits.filter(
    //   (tuit) =>
    //     sampleTuits.indexOf({ postedBy: tuit.postedBy, tuit: tuit.tuit }) >= 0
    // );
    const tuitsInserted = retrievedTuits.filter(
      tuit => sampleTuits.indexOf({tuit: tuit.tuit}) >=0
    )
    ...
    tuitsInserted.forEach((insertedTuit) => {
      ...
      // );
      const curUser = users.find(user => user._id === insertedTuit.postedBy);
      expect(curUser._id).toEqual(insertedTuit.postedBy);
      expect(`Hello ${curUser.username}`).toEqual(insertedTuit.tuit);
      console.log(curUser);
